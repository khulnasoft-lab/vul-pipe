...
echo "Running vul with options: ${ARGS}" "${artifactRef}"
...
if [[ ${template} == *"sarif"* ]]; then
  ...
  vul --quiet ${scanType} --format template --template ${template} --output ${output} $SARIF_ARGS ${artifactRef} $BITBUCKET_CLONE_DIR
fi
